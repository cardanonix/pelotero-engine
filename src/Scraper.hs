...
-- assembleFinalOutput :: IN.GameSchedule -> [Int] -> M.Map Int IN.GameData -> Text -> FinalOutput
-- assembleFinalOutput gameSchedule gameStatusList gameDataMap dateStr =
--   let playersOutput = ... -- Your existing logic to convert gameDataMap into OutputData
--       gamesOutput = M.fromList $ zip gameStatusList (repeat "F")  -- Example, adjust as needed
--       checksumOutput = calculateChecksum playersOutput  -- Adjust as needed
--   in FinalOutput playersOutput gamesOutput checksumOutput dateStr
